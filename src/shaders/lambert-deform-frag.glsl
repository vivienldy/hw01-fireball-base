...
void main()
{
    ...
    vec3 pos = vec3(fs_Pos.x + u_Time * u_Frequency, fs_Pos.y + u_Time * u_Frequency, fs_Pos.z + u_Time * u_Frequency);
    float fbmNoise = fbm(fs_UV); 
    float fbmNoise3D = fbm3d(pos) + 0.1;
    vec4 col = u_Color;
    ...
    out_Col = vec4(texture.x * intensity, texture.y * intensity, texture.z * intensity, 1.0);
